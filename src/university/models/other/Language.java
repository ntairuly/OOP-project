...
public enum Language {
	...
	Language() {
		...
		// Student.toString — теперь включает name
		add("Student.toString","Student[id=%s, name=%s, gpa=%.2f, year=%d, major=%s]",
				"Студент[id=%s, имя=%s, СРБ=%.2f, курс=%d, специальность=%s]",
				"Студент[id=%s, аты=%s, СРБ=%.2f, курс=%d, мамандық=%s]");

		// GraduateStudent.toString — теперь включает name
		add("GraduateStudent.toString", "GraduateStudent[id=%s, name=%s, gpa=%.2f, supervisor=%s]",
				"GraduateStudent[id=%s, имя=%s, СРБ=%.2f, научный руководитель=%s]",
				"GraduateStudent[id=%s, аты=%s, СРБ=%.2f, жетекші=%s]");

		// Student profile editing
		add("Student.editPrompt", "What to edit? (firstname / lastname / id)",
				"Что изменить? (firstname / lastname / id)",
				"Нені өзгерту керек? (firstname / lastname / id)");

		add("Student.newFirstName", "New first name: ",
				"Новое имя: ",
				"Жаңа аты: ");

		add("Student.newLastName", "New last name: ",
				"Новая фамилия: ",
				"Жаңа тегі: ");

		add("Student.newId", "New id: ",
				"Новый id: ",
				"Жаңа id: ");

		add("Student.profileUpdated", "Profile updated",
				"Профиль обновлен",
				"Профиль жаңартылды");

		add("Student.unknownField", "Unknown field",
				"Неизвестное поле",
				"Белгісіз өріс");
